...
| 6.7 | Five overlapping event list components | **Not a real finding — closed by measurement.** They compose, they don't overlap: `Events` (52 lines) is the page shell holding `EventSelector` (29, a semester dropdown), `EventsList` (92, the list) and `EventCreationModal`; `EventsList` renders `EventCards` (116, one card); `SelectedEvent` holds `AttendButton` and `PastEventExperience`. Every component is imported exactly once, and only `EventsList` renders a card. The audit inferred duplication from the file names; the one genuine orphan in that directory, `EventGallery`, was already deleted under §7.1 |
